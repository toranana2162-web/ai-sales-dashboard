...
- [x] #1-2 正式フォーマットのCSVアップロード・検証エラー表示（正式フォーマットの9月分CSV12件が「2025-09-01分のデータを12件登録しました」と成功表示、`cost`列欠損CSVで「必須列が不足しています: cost」とエラー表示を確認）
- [x] #3-4 DB保存・再アップロード時の置換（9月分12件→5件へ再アップロードしDB側の置換自体は正常と確認。あわせて、アップロード後にKPI表示が自動更新されない点を発見し、`app/page.tsx`に`refreshToken`を追加して修正。修正後、リロード無しで表示が正しく切り替わることを確認）
- [ ] #5-7 売上・粗利・リピート率の算出

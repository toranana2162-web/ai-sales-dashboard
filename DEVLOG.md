...
**Milestone 7（CI/CD）完了、ブランチ保護ルールの重大な不備を発見・修正**

- `pr-check.yml`にテスト実行ステップを追加し、初めて実際にブランチ・PRを作ってCIを通す流れを試した
  - CIが実際に失敗：`app/layout.tsx`の`LayoutProps<"/">`がNext.jsのビルド生成物依存で、ビルド前の型チェックでは存在せずエラー → プレーンな型に修正して解決
- ブランチ保護ルールを検証する過程で、実は一度も正しく保存されていなかったことが発覚
  - 調査の結果、classic branch protection・Rulesetsのどちらも「プライベートリポジトリ＋無料プラン」では実際には有効化されないというGitHubの制約が原因と判明
  - ユーザーの判断でリポジトリをPublicへ変更
  - 所有者によるルール迂回(bypass)や、誤って有効になっていた必須レビュー設定など、複数の細かい問題を一つずつ発見・修正
  - 最終的に、直接pushが実際に拒否されることを確認し、保護ルールが正しく機能することを実証
- Vercelとの自動デプロイ連携も、実際のPRマージを通じて動作確認
- Homebrewと GitHub CLI(`gh`)を新規導入。以降はPRの作成・チェック確認・マージをコマンドで行う運用に切り替え

## 2026-09-22

**Milestone 8（セキュリティ・非機能要件の確認）完了**

- APIキー・service role keyがコード・Git履歴に含まれていないこと、本番環境がHTTPSであること、
  `customer_id`以外の直接的個人情報を保存していないことを確認（いずれも問題なし）
- 1万行のダミーCSV（2025年8月分）を実際に生成してアップロードし、処理時間を計測
  - アップロード（検証・DB登録・集計反映）1.5秒＋KPI表示0.5秒＋推移表示0.4秒＝合計約2.4秒
  - REQ 21章の目標（10秒以内）を大きく下回ることを確認。AI分析(9.8秒)は同章で計測対象外と明記されており対象外
- Supabase Free tierの自動一時停止（7日間無操作で発生）について、実発生を7日間待つのは現実的でないため、
  公式仕様の確認とダッシュボード上のRestart/Pauseボタンの実在確認で代替し、復旧手順を運用ルールとしてPROGRESS.mdに記録
- これでMilestone 0〜8がすべて完了。次はMilestone 9（受け入れテスト、22項目）
